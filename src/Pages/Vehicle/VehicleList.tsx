  ...
  useEffect(() => {
    ...
    if(vehicles){
      vehicles.forEach((element) => {
        const response = element.brandAndModel.toLowerCase().includes(setSearch.toLowerCase());
        if(response === true){
          ...
          <ul className="dropdown-menu">
            {
              filterOptions.map((filterTypes, index) => {
                return (
                  <>
                    <li><a className="dropdown-item" onClick={() => handleFilterClick(index)}> {filterTypes} </a>
      ...
      <nav aria-label="...">
        <ul className="pagination">
          <li className="page-item">
            <a className="page-link" aria-disabled="true" onClick={() => handleFilterClick("Pagination",currentPage! - 1)}>Previous</a>
          </li>
          {
          ...
          <li className="page-item">
            <a className="page-link" onClick={() => handleFilterClick("Pagination",currentPage! + 1)}>Next</a>
          </li>
        </ul>
      </nav>
    </div>
