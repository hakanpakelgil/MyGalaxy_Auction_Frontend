...
import { Loader, ToastrNotify } from '../../../Helper'
...
function BidsDetail(props: { vehicleId: string }) {
    ...
    const createHubConnection = async () => {
        const hubConnection = new HubConnectionBuilder()
            .withUrl("https://localhost:7096/BidUpdate/Hub")
            ...
            <div className='bid-list'>
                <h1>{variable}</h1>
                {
                    bidState?.slice().sort((a:bidModel,b:bidModel) => b.bidAmount - a.bidAmount).map((bid: any,key:any) => {
                        ...
                        const shortDate = date.toLocaleString('tr-TR')
                        return (
                            <>
                                <div className='mt-4'>
                                    <div className='bid'>
                                        <span className='bid-number'> {key} </span>
                                        <span className='bid-date'> {shortDate} </span>
